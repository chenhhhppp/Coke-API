...
} from '@/services/cokeApi_backend/interfaceInfoController';
...
const TableList: React.FC = () => {
  ...
  // 模态框的变量在 TableList组件里，所以把增删改节点都放进来
  /**
   * @en-US Add node
   * @zh-CN 添加节点
   * @param fields
   */
  // 把参数的类型改成 InterfaceInfo
  ...
  return (
    <PageContainer>
      ...
      <CreateModal
        columns={columns}
        // 当取消按钮被点击时，设置更新模态框为false以隐藏模特窗口
        onCancel={() => {
          handleModalOpen(false);
        }}
        // 当用户点击提交按钮之后，调用handleAdd函数提交的数据，去请求后端添加数据（这里的报错不用管，可能里面的组件属性和外层不一致）
        onSubmit={(values) => {
          handleAdd(values);
        }}
        // 根据更新窗口的值决定模态框窗口是否显示
